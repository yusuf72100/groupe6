...
/**
 * Classe qui correspond à interface graphique d'une partie joué de Slitherlink
 *
 * @author Yusuf
 */
public class GridMenu implements Menu {
  ...
  /**
   * L'anchorPane qui contient les éléments de l'interface graphique
   */
  private final AnchorPane anchorPane;

  /**
   * Le stackPane qui contient l'anchorPane
   */
  ...
  /**
   * Le thread du chronomètre
   */
  private final Thread thread;

  /**
   * Le label du chronomètre
   */
  private final Label chronoLabel;

  /**
   * Le conteneur du label du mode hypothèse
   */
  private final HBox hypotheseHbox;

  /**
   * Le label du mode hypothèse
   */
   ...
   * @param buttonCss le css du bouton
   * @param centerCss le css du centre
   */
  ...
  /**
   * Méthode qui change la couleur des boutons (traits) des cellules voisines
   *
   * @param y la position y de la cellule
   * @param x la position x de la cellule
   * @param css le css à appliquer
   */
  public void setCellulesAdjacentesCss(int y, int x, String css) {

    if ( estDansGrille(y, x-1) ) {
      if(celluleNodes[y][x-1] != null) celluleNodes[y][x-1].changeButtonCss(3, css);
    }

    if ( estDansGrille(y, x+1) ) {
      if(celluleNodes[y][x+1] != null) celluleNodes[y][x+1].changeButtonCss(2, css);
    }

    if ( estDansGrille(y-1, x) ) {
      if(celluleNodes[y-1][x] != null) celluleNodes[y-1][x].changeButtonCss(1, css);
    }

    if ( estDansGrille(y+1, x) ) {
      if(celluleNodes[y+1][x] != null) celluleNodes[y+1][x].changeButtonCss(0, css);
    }
  ...
  public void resetCellulesAdjacentesCss(int y, int x) {
    ...
    if ( estDansGrille(y, x-1) ) {
      if(celluleNodes[y][x-1] != null) celluleNodes[y][x-1].resetButtonCss(3);
    }

    if ( estDansGrille(y, x+1) ) {
      if(celluleNodes[y][x+1] != null) celluleNodes[y][x+1].resetButtonCss(2);
    }

    if ( estDansGrille(y-1, x) ) {
      if(celluleNodes[y-1][x] != null) celluleNodes[y-1][x].resetButtonCss(1);
    }

    if ( estDansGrille(y+1, x) ) {
      if(celluleNodes[y+1][x] != null) celluleNodes[y+1][x].resetButtonCss(0);
    }
  ...
  private void initCellules(int l, int L, Double w, Double h) {
    ...
    for (int i = 0; i < l; i++) {
      for (int j = 0; j < L; j++) {
        celluleNodes[i][j] = new CelluleNode(this.cellulesData[i][j].getValeur(), this.cellulesData[i][j].getCotes());
        celluleNodes[i][j].setPrefSize(Menu.toPourcentWidth(500.0 / this.longueur, w), Menu.toPourcentWidth(500.0 / this.longueur, w));
      }
    }
  }

  /**
   * Méthode qui sert à afficher le puzzle
   */
  private void afficher() {
    ...
    for (int i = 0; i < celluleNodes.length; i++) {
      ...
      for (int j = 0; j < celluleNodes[i].length; j++) {
        // Coins
        this.gridPane.add(celluleNodes[i][j].getCoin(0), j * 2, i * 2);            // top left
        this.gridPane.add(celluleNodes[i][j].getCoin(1), j * 2 + 2, i * 2);        // top right
        this.gridPane.add(celluleNodes[i][j].getCoin(2), j * 2, i * 2 + 2);        // bottom left
        this.gridPane.add(celluleNodes[i][j].getCoin(3), j * 2 + 2, i * 2 + 2);    // bottom right
        ...
        if (i == 0) {
          celluleNodes[i][j].getButton(0).setGraphic(celluleNodes[i][j].getImage(0));
          celluleNodes[i][j].getButton(0).setContentDisplay(ContentDisplay.CENTER);
          this.gridPane.add(celluleNodes[i][j].getImage(0), j * 2 + 1, i * 2);   // top
          this.gridPane.add(celluleNodes[i][j].getButton(0), j * 2 + 1, i * 2);   // top
          celluleNodes[i][j].getButton(0).setOnAction(new CelluleButtonEventHandler(i,j, this.cellulesData));
        }
        celluleNodes[i][j].getButton(1).setGraphic(celluleNodes[i][j].getImage(1));
        celluleNodes[i][j].getButton(1).setContentDisplay(ContentDisplay.CENTER);
        this.gridPane.add(celluleNodes[i][j].getCenterPane(), j * 2 + 1, i * 2 + 1);   // center
        this.gridPane.add(celluleNodes[i][j].getImage(1), j * 2 + 1, i * 2 + 2);   // bottom
        this.gridPane.add(celluleNodes[i][j].getButton(1), j * 2 + 1, i * 2 + 2);   // bottom
        celluleNodes[i][j].getButton(1).setOnAction(new CelluleButtonEventHandler(i,j, this.cellulesData));

        // Avoid vertical bar duplication
        if(j == 0){
          celluleNodes[i][j].getButton(2).setGraphic(celluleNodes[i][j].getImage(2));
          celluleNodes[i][j].getButton(2).setContentDisplay(ContentDisplay.CENTER);
          this.gridPane.add(celluleNodes[i][j].getImage(2), j * 2, i * 2 + 1);   // left
          this.gridPane.add(celluleNodes[i][j].getButton(2), j * 2, i * 2 + 1);   // left
          celluleNodes[i][j].getButton(2).setOnAction(new CelluleButtonEventHandler(i,j, this.cellulesData));
        }
        celluleNodes[i][j].getButton(3).setGraphic(celluleNodes[i][j].getImage(3));
        celluleNodes[i][j].getButton(3).setContentDisplay(ContentDisplay.CENTER);
        this.gridPane.add(celluleNodes[i][j].getImage(3), j * 2 + 2, i * 2 + 1);   // right
        this.gridPane.add(celluleNodes[i][j].getButton(3), j * 2 + 2, i * 2 + 1);   // right
        celluleNodes[i][j].getButton(3).setOnAction(new CelluleButtonEventHandler(i,j, this.cellulesData));
      }
    }
  }
  ...
  public void updatePuzzle() {
    ...
    for ( int y = 0; y < this.largeur; y++ ) {
      for (int x = 0; x < this.longueur; x++) {
        celluleNodes[y][x].updateCotes(cellulesData[y][x].getCotes());
      }
    }
  }

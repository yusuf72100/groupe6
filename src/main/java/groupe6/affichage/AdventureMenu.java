...
/**
 * Classe qui correspond au menu du mode aventure qui permet de choisir sur quel puzzle l'utilisateur veut jouer
 *
 * @author Yamis
 */
public class AdventureMenu extends ClassicModeMenu {

  /**
   * Label qui contient le temps limite
   */
  protected static Label labelNivAventure;
  ...
  public static StackPane getMenu(Double w, Double h) {
    ...
    btnJouer.setOnMouseClicked(e -> {
      Main.lancerPartie(difficulteSelectionne, numeroPuzzleSelectionne, ModeJeu.AVENTURE);
    });
    ...
    for (int i = 0; i < 3; i++) {
      ...
      // Sinon, on affiche les previews des puzzles disponibles pour la difficulté
        ...
        for (int j = 0; j < nbPuzzleParDifficulte ; j++) {
          ...
          imgPreviewPuzzle.setFitWidth(0.25 * windowWidth);
          imgPreviewPuzzle.setFitHeight(0.25 * windowWidth);
          ...
          previewPane.getChildren().add(imgPreviewPuzzle);

          if ( !puzzleUnlocked ) {
            // Créer un rectangle semi-transparent
            Rectangle overlay = new Rectangle(0.25 * windowWidth, 0.25 * windowWidth, Color.rgb(0, 0, 0, 0.5));

            // Créer une croix rouge
            Label cross = new Label("✖");
            Menu.adaptTextSize(cross, 60, windowWidth, windowHeight);
            cross.setStyle(
                cross.getStyle() +
                    "-fx-text-fill: red;"
            );
            ...
            overlay.setOnMouseClicked(e -> {
              Main.afficherPopUpInformation(
                  "Puzzle verrouillé",
                  "Ce puzzle est verrouillé, vous devez terminer les puzzles précédents pour le déverrouiller.",
          ...
          HBoxPreviewContainer.getChildren().add(previewPane);
          ...
          imgPreviewPuzzle.setOnMouseClicked(e -> {
            ...
            if ( previewSelectionne == imgPreviewPuzzle && puzzleUnlocked) {
    ...
    StackPane.setMargin(scrollPane, new javafx.geometry.Insets(0.1 * windowWidth,  0, 0.17 * windowHeight, 0.08 * windowWidth));

    // Gestion du titre
    title = new Label("Aventure");
    title.getStyleClass().add("title");
    StackPane.setAlignment(title, Pos.TOP_CENTER);

    // Niveau d'aventure
    int niveauAventure = Launcher.catalogueProfils.getProfilActuel().getNiveauAventure() + 1;
    labelNivAventure = new Label("Niveau : " + niveauAventure);
    Menu.adaptTextSize(labelNivAventure, 45, windowWidth, windowHeight);
    StackPane.setMargin(labelNivAventure, new javafx.geometry.Insets(0.09 * windowHeight, 0, 0, 0));
    StackPane.setAlignment(labelNivAventure, Pos.TOP_CENTER);
